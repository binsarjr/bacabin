...
class MirrorKomik extends BaseKomik {
	...
	async chapters(link: string): Promise<Chapter[]> {
		...
		$('#Chapter li').each((i, el) => {
			chapters.push({
				...
				title: `Chapter ${$(el).find('a').text().replace(/^ch\.\s+/i, '')}`
			})
		})
		return chapters
	}
	...
	async read(chapter_link: string): Promise<ReadChapter | null> {
		...
		const title = $('#Deskripsi h1')
			.text()

		let prev = null
		let next = null
		const showLink = this.toLink($('#Chapter_Lainnya tr:last-child .judulseries a').attr()!['href'])
		...
		let [_, id, id2] = $('#thisch').attr('onclick')?.replace(/^\w+/, '').replace(/^\(/, '').replace(/\)$/, '').split(',') as [string, string, string]
		id = id.replace(/'/g, '')
		id2 = id2.replace(/'/g, '')

		const chapLink = `https://mirrorkomik.net/Chapter/listchap,${id},${id2}`
		const chapterImages = (await this.request.get(chapLink, {
			headers: {
